...
class GridNavigator:
    ...
    def flag_danger(self, pos: tuple):
        ...
        # 1. Heavy penalty to trap cell itself
        for a in range(4):
            self._q[x][y][a] = -1000.0

        # 2. Stronger penalty to neighboring cells
        for dx, dy in [(1,0), (-1,0), (0,1), (0,-1)]:
            nx, ny = x + dx, y + dy

            if 0 <= nx < self._q.shape[0] and 0 <= ny < self._q.shape[1]:
                for a in range(4):
                    self._q[nx][ny][a] -= 25.0

        # 3. Block all actions leading INTO the trap
        for prev_pos, edges in self._known_edges.items():

            for a_idx, nxt in list(edges.items()):

                if nxt == pos:

                    px, py = prev_pos

                    # extremely strong punishment
                    self._q[px][py][a_idx] = -5000.0

                    # permanently block this action
                    if prev_pos not in self.wall_map:
                        self.wall_map[prev_pos] = set()

                    self.wall_map[prev_pos].add(a_idx)
	# 4. Remove dangerous transitions from replay buffer
        clean_buf = deque(maxlen=self._replay_buf.maxlen)

        for item in self._replay_buf:
            p, a, r, nxt, err, terminal = item

            if nxt == pos:
                continue

            clean_buf.append(item)

        self._replay_buf = clean_buf

        # 5. Invalidate BFS cache
    ...
    def _greedy_action(self, pos: tuple) -> int:
        ...
        if not self.goal_found:
            ...
            def ucb(a: int) -> float:
                ...
                nx = min(max(x + dx, 0), max_x - 1)
                ny = min(max(y + dy, 0), max_y - 1)
                bonus = 4.0 * math.sqrt(log_t / (1 + self._visit_counts[nx][ny]))
